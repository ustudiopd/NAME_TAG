...
Supabase JS 클라이언트 호환성을 위해 `public` 스키마에 뷰를 생성합니다:
...
export const supabaseAdmin = createClient(
  ...
  process.env.SUPABASE_SERVICE_ROLE_KEY!,
  {
    db: { schema: '{project_name}' }  // 기본 스키마 설정
  }
);

// 사용 시
const { data } = await supabaseAdmin
  .from('{table_name}')  // 스키마 자동 적용
  .select('*');
```
...
```typescript
const { data } = await supabaseAdmin
  .schema('{project_name}')  // 스키마 명시
  .from('{table_name}')
  .select('*');
```

**옵션 3: Public 뷰 사용 (기존 코드 유지)**

```typescript
// Public 뷰를 통해 접근 (코드 수정 최소화)
const { data } = await supabaseAdmin
  .from('{table_name}')  // public.{table_name} 뷰 사용
  .select('*');
```
...
// 또는 public 래퍼 함수 사용
const { data } = await supabaseAdmin.rpc('{function_name}', {
  param1: value1,
  param2: value2
});
```
...
```typescript
// lib/supabase-{project-name}.ts
import { createClient } from '@supabase/supabase-js';

export const supabase{ProjectName} = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!,
  {
...
export function getSupabaseClient(schema?: string) {
...
// 사용
const hddClient = getSupabaseClient('hdd');
const vibememoryClient = getSupabaseClient('vibememory');
```
...
1. Public 뷰 생성 (필요한 경우)
2. 코드에서 `hdd` 스키마 사용하도록 수정
3. 테스트 및 검증
